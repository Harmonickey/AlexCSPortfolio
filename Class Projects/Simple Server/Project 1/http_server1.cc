...
int main(int argc,char *argv[])
{
  ...
  if (rc<0) { printf("Error:bind failure\n"); }
  /* start listening */
  rc = minet_listen(sock,BACKLOG);
  if (rc<0) { printf("Error:listening failure\n");}
...
int handle_connection(int sock2)
{
  ...
  int filename_end = request.find("HTTP/1.0") - 2; //find end position of filename
  int filename_len = filename_end - 4; //computer filename length
  if (filename_len > FILENAMESIZE) {
    printf("Error: file name is too long\n");
    return -1;
  }
  string filename_s = request.substr(5,filename_len); //retrieve filename from request
  //strcpy(filename,filename_s.c_str()); //copy string filename into char* filename

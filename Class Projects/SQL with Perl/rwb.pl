...
#
#my $dbuser=$ENV{USERNAME};
#my $dbpasswd=$ENV{PASSWORD};
my $dbuser='ama610';
my $dbpasswd='zMz67zgfM';
...
if ($action eq "base") { 
  ...
  print "<div id=\"results\" style=\"width:100\%; height:10\%\"></div>";
  #
  #
  ...
  eval { @cyles_arr = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.candidate_master ORDER BY cycle ASC"); };

  print "<div>";
  print "<input type=\"checkbox\"id=\"committee\" name=\"committee\" value=\"Committee\" onclick=\"ViewShift()\">Committee";
  print "<input type=\"checkbox\"id=\"candidate\" name=\"candidate\" value=\"Candidate\" onclick=\"ViewShift()\">Candidate";
  print "<input type=\"checkbox\"id=\"individual\" name=\"individual\" value=\"Individual\" onclick=\"ViewShift()\">Individual";
  if(UserCan($user, "query-opinion-data"))
  {
       print "<input type=\"checkbox\"id=\"opinion\" name=\"opinion\" value=\"Opinion\" onclick=\"ViewShift()\">Opinion";
  }
  foreach (@cyles_arr)
  {
    print "<input class=\"cycles\" type=\"checkbox\" id=@{$_} name=@{$_} value=@{$_} onclick=\"ViewShift()\">@{$_}";
  }

  print "<p id='lat' hidden></p>";
  print "<p id='long' hidden></p>";
  
  print "</div>";
  
  print "<div id=\"summaries\" style=\"width:100\%; height:10\%\"></div>";
  #
...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
  ...
  if ($user eq "anon") {
    ...
    # if (UserCan($user, "query-opinion-data")){
    #   print "<p><a href=\"rwb.pl?act=query-opinion-data\">Query Opinion Data</a></p>";   
    # }
...
if ($action eq "near") {
  ...
  print start_form(-id=>'myIndvData'),
        hidden(-id=>'demIndv', -default=>[$repSum]),
          hidden(-id=>'repIndv', -default=>[$demSum]),
            end_form, hr;
 ...
 #  }
  if ($what{individuals}) {
    my ($str,$error) = IndAggr($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
  print "<h2>Nearby individuals</h2>$str";
      } else {
 ...
 if ($what{opinions}) {
    my ($str,$error) = OpAggr($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
  print "<h2>Nearby opinions</h2>$str";
      } else {
...
if ($action eq "give-opinion-data") { 
  if (!UserCan($user,"give-opinion-data")) {
    ...
    if (!$run) {
      ...
      print "function getCurrentPosition()";
      print "{";
      print "  navigator.geolocation.getCurrentPosition(Next);";
      print "}";

      print "function Next(location)";
      print "{";
      print "  document.getElementById('lat').value = location.coords.latitude;";
      print "  document.getElementById('long').value = location.coords.longitude;";
      print "}";

      print "</script>";


      print start_html(-onload=>"getCurrentPosition()");
      print h2("Choose your political opinion");
      print start_form(-name => 'Add Opinion'),
        radio_group(-name=>'pol_op', -value=>['Red', 'Blue', 'White']),
          hidden(-name=>'run', -default=>['1']),
            hidden(-name=>'op_lat', -id=>'lat'),
              hidden(-name=>'op_long', -id=>'long'),
      ...
      my $pol_op = param('pol_op');

      my $lat = param('op_lat');
      my $long = param('op_long');
      ...
      eval {
        ExecSQL($dbuser,$dbpasswd, "insert into rwb_opinions (submitter, color, latitude, longitude) values(?,?,?,?)", 
                  undef,$user,$color_hash{$pol_op}, $lat, $long);
      };
    }
...
if ($action eq "add-user") { 
  if (!UserCan($user,"add-users") && !UserCan($user,"manage-users")) { 
    ...
    if (defined($access_token) && !$run) {
      if (ValidUser($access_token, '12345678')) {
                      ...
                      p,
                        hidden(-name=>'run',-default=>['1']),
                          hidden(-name=>'act',-default=>['add-user']),
                            hidden(-name=>'access_token',-default=>["$access_token"]),
                              submit,
                               end_form,
                                 hr;
      } else {
        print h2('Your link can only be used once, you must be invited again.');
      }
      ...
      my $error;
      my @actions = GetPermissions($access_token);
      UserDelete($access_token);
      $error=UserAdd($name,$password,$email,$user);
      if ($error) {
        print "Can't add user because: $error";
      }
      ##Then add in the permissions...
      foreach (@actions)
      {
        $error=GiveUserPerm($name, $_);
        if ($error) {
          print "Can't add permissions to $name because: $error";
        }
      }
    } elsif (!$run) { 
      print start_form(-name=>'AddUser'),
...
if ($action eq "invite-user") { 
    ...
    if (!$run) {
      ...
      my @perms = GetPermissions($user);

      print start_form(-name=>'InviteUser'),
        h2('Invite User'),
          "Email: ", textfield(-name=>'email'),
            p,
              "Select which permissions to give to the user:",
                        p,
                            checkbox_group(-name=>'perms', -values=>\@perms),
                              p,
                                hidden(-name=>'run',-default=>['1']),
      ...
      my @vals=param('perms');
      my $error;
          $error=UserInvite($email,$user, @vals);
      if ($error) {
...
sub IndAggr {
  ...
  my $numInd = @rows;
  #if the data set is not big enough
  while ($numInd<2 && &count < 6)
  {
    
    $latneN = $latne + $latne/2 - $latsw/2;
    $longneN = $longne + $longne/2 - $longsw/2;
    $latswN = $latne + $latne/2 - $latsw/2; 
    $longswN = $longsw + $longsw/2 - $longne/2;
    eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt from cs339.individual natural join cs339.ind_to_geo where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
    };
    $numInd = @rows;
...
@rows_demIndiv = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt)
                  from (select unique cmte_id, transaction_amnt
                        from cs339.individual natural join cs339.ind_to_geo 
                        where cycle IN ?
                        and latitude>?
                        and latitude<? 
                        and longitude>? 
                        and longitude<?) natural join cs339.committee_master
                  WHERE cmte_pty_affiliation in ('DEM', 'D', 'Dem')",
                  "COL", $cycle,$latsw,$latne,$longsw,$longne);
...
@rows_demIndiv = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt)
                  from (select unique cmte_id, transaction_amnt
                        ...
                        and longitude>? 
                        and longitude<?) natural join cs339.committee_master
                  WHERE cmte_pty_affiliation in ('REP', 'R', 'Rep')",
                  "COL",$cycle,$latsw,$latne,$longsw,$longne);
  ...
  if ($@) { 
    ...
    $repSum = $rows_repIndiv[0];
    $demSum = $rows_demIndiv[0];
     print start_form(-id=>'myIndviData'),
        hidden(-id=>'demIndv', -default=>[$repSum]),
          hidden(-id=>'repIndv', -default=>[$demSum]),
            end_form, hr;
    ...
    if ($format eq "table") { 
      ...
      return (MakeRaw("individual_aggr","2D",@rows_repIndiv),$@);
    }
  }
}
...
sub OpAggr {
  ...
  eval { 
    ...
    @rows = ExecSQL($dbuser, $dbpasswd, "SELECT avg(color) FROM rwb_opinions WHERE latitude>? AND latitude<? AND longitude>? AND longitude<?", 'COL' ,$latsw,$latne,$longsw,$longne);
    @rows2 = ExecSQL($dbuser, $dbpasswd, "SELECT stddev(color) FROM rwb_opinions WHERE latitude>? AND latitude<? AND longitude>? AND longitude<?", 'COL' ,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) { 
    return (undef,$@);
      ...
      print start_form(-id=>'myOpData'),
        hidden(-id=>'opinionAverage', -default=>[$rows[0]]),
          hidden(-id=>'opinionStddev', -default=>[$rows2[0]]),
            end_form, hr;
...
sub UserInvite {
 my ($emailAddress, $user, @vals) = @_;
 my $range = 100000;

 my $pass = int(rand($range));

 my $link = CreateLink($pass);

 my $subject="Your're Invited to Create a New Account!";

 my $content="Click on the link below to create your account.\n";
 $content.="You will create your own username and password from there.\n";
 $content.=$link;

 my $to = $emailAddress;
 my $from = 'AlexanderAyerdi2015@u.northwestern.edu';
 my $message = $content;
 ...
 ##create the pseudo user
 my $error=UserAdd($pass, '12345678', $emailAddress, $user);

 if ($error) {
   print "Can't add user from invite because: $error";
   return $@;
 }

 ##add perms to the pseudo user...
 foreach (@vals)
 {
   $error=GiveUserPerm($pass, $_);
   if ($error) {
     print "Can't add permissions to $pass because: $error";
     return $@;
   }
 }
...
sub CreateLink {
  ...
  my $base_link = "http://murphy.wot.eecs.northwestern.edu/~ama610/rwb/rwb.pl?";
  my $endpoint = "act=add-user&user=root&access_token=$pass";

  return $base_link . $endpoint;
}

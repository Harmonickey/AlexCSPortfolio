...
void debug_output(const ConnectionList<TCPState>::iterator cs, unsigned char flags, unsigned int seqnum, unsigned int acknum);
...
int main(int argc, char *argv[])
{
  ...
  cout<<"We are about to process event!\n";
...
void muxHandler(const MinetHandle &mux, const MinetHandle &sock, ConnectionList<TCPState> &clist)
{
  cout<<"clist has size: "<<clist.size()<<endl;
  cout<<"We are now in muxHandler"<<endl;
	Packet p;
	Packet out_packet; 
	...
	//unsigned iphlen=IPHeader::EstimateIPHeaderLength(p)-40;
	...
	*/
	...
	cout<<"we are about to find matching connections"<<endl;
	 ...
	 if (cs!=clist.end()) {
	   ...
	   switch (state) {
		   ...
		   case LISTEN:
			...
			//we may want to check for FIN is case there needs to be an RST in the connection (reset)
		     ...
		     (*cs).state.SetLastSent(100); 
		     (*cs).state.SetLastRecvd(100);
		     (*cs).state.SetLastAcked(100);
		     
		     debug_output(cs,flags,seqnum,acknum);
		     if (IS_SYN(flags)) {
		   ...
		   case SYN_RCVD:
		     ...
		     debug_output(cs,flags,seqnum,acknum);
		     if ((*cs).state.GetLastRecvd()+1 == seqnum) {  //p266 (3rd arrow)
		     	if (IS_ACK(flags)) {
			    	printf("Established\n");
		   ...
		   case SYN_SENT:
		     //handle state SYN_SENT;
				...
				cout << "Getlastsent + 1 == acknum" << endl;
		   ...
		   case ESTABLISHED:
			...
			if (IS_ACK(flags)) {
				...
				if (acknum == (*cs).state.GetLastSent() - 1) {
				  (*cs).bTmrActive = false;
				} else {
				  (*cs).bTmrActive = true;
				  (*cs).timeout = Time() + 50;
	   ...
	   cout<<"No matching connection"<<endl;
	 }
}
...
void sockHandler(const MinetHandle &mux, const MinetHandle &sock, ConnectionList<TCPState> &clist)
{
  cout<<"clist has size: "<<clist.size()<<endl;
    ...
    cout<<"We are in socket handler"<<endl;
    switch(s.type)
    {
	case CONNECT:
	{   
	    cout<<"Socket CONNECT request"<<endl;
	    Packet out_packet;

	    unsigned int timerTries = 3;
	    
	    TCPState newTCPState = TCPState(1, SYN_SENT, timerTries);  //SYN_SENT because diagram for *active open ; also p15 Minet
	     
	    ConnectionToStateMapping<TCPState> constate;
	    constate.connection = s.connection;
            constate.timeout = Time();
	...
	case ACCEPT:
	{   
	  cout<<"Socket ACCEPT request"<<endl;
	  //cout<<"clist has size: "<<clist.size()<<endl; 
	   unsigned int state = (*cs).state.GetState();
	...
	case WRITE:
	 {
	   cout<<"Socket WRITE"<<endl;
	    ...
	    {
		unsigned dataSize = MIN_MACRO(IP_PACKET_MAX_LENGTH-IP_HEADER_BASE_LENGTH-TCP_HEADER_BASE_LENGTH,
                                     s.data.GetSize());

	        Packet dataSend = Packet(s.data.ExtractFront(dataSize));
		
		//turn on timer if last_acked = last_sent-1
		...
		packetMaker(dataSend, *cs, dataSize, S_ACK);
...
void packetMaker(Packet& packet, ConnectionToStateMapping<TCPState>& constate, int size_of_data, int whichFlag)
{
  ...
  tcp.SetAckNum(constate.state.GetLastRecvd()+1, packet);
  tcp.SetSeqNum(constate.state.GetLastSent(), packet);

  //---for debug use---
  cout<<"------ We are sending out a packet! -----"<<endl;
  cout<<"AckNum being sent out: "<<constate.state.GetLastRecvd()+1<<endl;
  cout<<"SeqNum being sent out: "<<constate.state.GetLastSent()<<endl;
  printf("Flags being sent out: %d \n",flags);
  cout<<"-----------------------------------------"<<endl;
  //-------------------
